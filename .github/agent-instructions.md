...
## Project Manager & Dependency Parsing (`internal/pm`)
The PM parses logic specifications (`types.Specification`) into atomic tasks (`WorkPackage`).
...
## Execution Security & Validation (`internal/validator`)
- **PlanValidator**: Phase 1 ensures structural existence (goal, acceptance criteria). Phase 2 offloads to LLM inference to score logic flow.
- **WorkValidator**: Bounded severely. When executing tasks against shell commands internally (e.g. assessing completion), strictly abide by the internal `allowlist` (e.g., `go`, `make`, `pytest`, `grep`).
