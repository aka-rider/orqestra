...
// Package detect discovers tool installations and returns seatbelt.Snapshot values.
...
func DetectClaude(home string, binary string) (seatbelt.Snapshot, error) {
	...
	for _, opt := range optionals {
		if err := p.AllowOptional(opt.path, opt.perm); err != nil {
			return seatbelt.Snapshot{}, fmt.Errorf("detect claude optional path %q: %w", opt.path, err)
		}
	}
...
func DetectHomebrew(home string) (*seatbelt.Snapshot, error) {
	...
	if err := p.AllowOptional(binDir, seatbelt.Exec); err != nil {
		return nil, fmt.Errorf("detect homebrew bin: %w", err)
	}

	// Homebrew opt/lib for dynamic linking
	for _, sub := range []string{"opt", "lib"} {
		if err := p.AllowOptional(filepath.Join(basePath, sub), seatbelt.Read); err != nil {
...
func DetectDocker(home string) (*seatbelt.Snapshot, error) {
	p := seatbelt.NewToolProfile("docker", home)
	found := false
	...
	if err := p.AllowOptional("~/.docker/bin", seatbelt.Exec); err != nil {
	...
	if err := p.AllowOptional("~/.docker/config.json", seatbelt.Read); err != nil {
		return nil, fmt.Errorf("detect docker config: %w", err)
	}

	// Docker contexts
	if err := p.AllowOptional("~/.docker/contexts", seatbelt.Read); err != nil {
		return nil, fmt.Errorf("detect docker contexts: %w", err)
	}

	// Docker socket dir (needs write for docker commands)
	if err := p.AllowOptional("~/.docker/run", seatbelt.Write); err != nil {
	...
	for _, sock := range []string{"/var/run/docker.sock", "/private/var/run/docker.sock"} {
		if info, err := os.Stat(sock); err == nil && !info.IsDir() {
			if err := p.Allow(sock, seatbelt.Write); err != nil {
...
func DetectGit(home string) (*seatbelt.Snapshot, error) {
	p := seatbelt.NewToolProfile("git", home)
	found := false
	...
	if err := p.AllowOptional("~/.ssh/known_hosts", seatbelt.Read); err != nil {
	...
	if err := p.AllowOptional(cltPath, seatbelt.Exec); err != nil {
...
func DetectNPM(home string) (*seatbelt.Snapshot, error) {
	p := seatbelt.NewToolProfile("npm", home)
	found := false
	...
	if info, err := os.Stat(nvmDir); err == nil && info.IsDir() {
		if err := p.Allow("~/.nvm", seatbelt.Exec); err != nil {
