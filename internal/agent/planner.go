...
func (p *Planner) Refine(ctx context.Context, researcherDraft string) (RawPlan, error) {
	...
	if err != nil {
		return RawPlan{}, fmt.Errorf("planner refine: %w", err)
	}
	return p.parsePlanResult(result)
}

// RefineStreaming is like Refine but streams output.
func (p *Planner) RefineStreaming(ctx context.Context, researcherDraft string, stdout io.Writer) (RawPlan, error) {
	prompt := "Refine this researcher draft into a final implementation plan:\n\n" + researcherDraft
	result, err := p.runner.RunStreaming(ctx, prompt, p.cfg.SystemPrompt, stdout)
	if err != nil {
		return RawPlan{}, fmt.Errorf("planner refine streaming: %w", err)
	}
	return p.parsePlanResult(result)
}

// RefineWithComments takes a previous plan and human comments, producing a revised plan.
func (p *Planner) RefineWithComments(ctx context.Context, previousPlan, comments string) (RawPlan, error) {
	prompt := fmt.Sprintf("Revise this plan based on the reviewer's comments:\n\n## Current Plan\n\n%s\n\n## Reviewer Comments\n\n%s", previousPlan, comments)
	result, err := p.runner.RunPrint(ctx, prompt, p.cfg.SystemPrompt)
	if err != nil {
		return RawPlan{}, fmt.Errorf("planner refine with comments: %w", err)
	}
	return p.parsePlanResult(result)
}

// RefineWithCommentsStreaming is like RefineWithComments but streams output.
func (p *Planner) RefineWithCommentsStreaming(ctx context.Context, previousPlan, comments string, stdout io.Writer) (RawPlan, error) {
	prompt := fmt.Sprintf("Revise this plan based on the reviewer's comments:\n\n## Current Plan\n\n%s\n\n## Reviewer Comments\n\n%s", previousPlan, comments)
	result, err := p.runner.RunStreaming(ctx, prompt, p.cfg.SystemPrompt, stdout)
	if err != nil {
		return RawPlan{}, fmt.Errorf("planner refine with comments streaming: %w", err)
	}
	return p.parsePlanResult(result)
}

// parsePlanResult extracts markdown from a run result and performs basic sanity checks.
func (p *Planner) parsePlanResult(result harness.RunResult) (RawPlan, error) {
	...
	if !strings.HasPrefix(md, "# Plan") {
		return RawPlan{}, fmt.Errorf("planner output does not start with '# Plan' (got: %s)", truncateRaw(md, 100))
	}
	if !strings.Contains(md, "## Work Packages") {
		return RawPlan{}, fmt.Errorf("planner output missing '## Work Packages' section")
	}
