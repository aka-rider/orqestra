...
func New(client *harness.Client, cfg *config.PlannerConfig) *Planner {
	return &Planner{client: client, cfg: cfg}
}
...
func (p *Planner) Plan(ctx context.Context, prompt string) (types.Result[types.Specification], error) {
	resp, err := p.client.Run(ctx, prompt, p.cfg.SystemPrompt)
	if err != nil {
