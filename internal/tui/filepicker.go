...
func (m Model) handleFilePickerKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	switch msg.Type {
	case tea.KeyEsc:
		// Cancel picker, remove the '@' + query from prompt
		m.fp.stopScan()
		m.fpActive = false
		val := m.prompt.Value()
		if m.fpAtStart < len(val) {
			m.prompt.SetValue(val[:m.fpAtStart] + val[m.fpAtStart+1+len(m.fpQuery):])
		}
		m.fpQuery = ""
		return m, nil
	...
	case tea.KeyEnter:
		...
		if sel != "" {
			val := m.prompt.Value()
			before := val[:m.fpAtStart]
			after := ""
			end := m.fpAtStart + 1 + len(m.fpQuery)
			if end < len(val) {
				after = val[end:]
			}
			m.prompt.SetValue(before + sel + " " + after)
		}
		m.fpQuery = ""
		return m, nil
	...
	case tea.KeyDown:
		if m.fp.cursor < len(m.fp.filtered)-1 {
			m.fp.cursor++
		}
		return m, nil
	...
	case tea.KeyBackspace:
		if len(m.fpQuery) > 0 {
			m.fpQuery = m.fpQuery[:len(m.fpQuery)-1]
			m.fp.refilter(m.fpQuery)
		} else {
			// Backspace at empty query cancels picker
			m.fp.stopScan()
			m.fpActive = false
			val := m.prompt.Value()
			if m.fpAtStart < len(val) {
				m.prompt.SetValue(val[:m.fpAtStart] + val[m.fpAtStart+1:])
			}
		}
		return m, nil
...
// activateFilePicker initialises the file picker overlay after '@' is typed in the prompt.
func (m Model) activateFilePicker(pendingCmd tea.Cmd) (tea.Model, tea.Cmd) {
	cwd, err := os.Getwd()
	if err != nil {
		return m, pendingCmd
	}
	contentWidth := max(1, int(float64(m.width)*splitRatio))
	contentHeight := max(1, m.height-constHeaderHeight-constPromptInputHeight-constFooterHeight)
	m.fp = newFilePicker(cwd, contentWidth, contentHeight)
	m.fpActive = true
	m.fpAtStart = len(m.prompt.Value()) - 1 // '@' is the last char typed
	m.fpQuery = ""
	scanCmd := m.fp.startScan()
	if pendingCmd != nil {
		return m, tea.Batch(pendingCmd, scanCmd)
	}
	return m, scanCmd
}
